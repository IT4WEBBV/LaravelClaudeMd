...
- **Update the changelog**: When creating a PR, update the project's changelog file with a summary of the changes. Always check the latest version tag first with `git tag --sort=-v:refname | head -5` to determine the correct next version number.
