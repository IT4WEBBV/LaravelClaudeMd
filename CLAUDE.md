...
- **Never address another human without my explicit permission**: writing on PRs and issues is fine — descriptions, titles, status notes, findings, anything documenting my own work. What needs my approval first is *talking to someone*: replies to reviewers, answers to their questions, responses to a comment aimed at me, and reactions (👍 etc.) on someone else's comment. This holds even when the answer is ready and correct — colleagues read it as coming from me, so I decide what gets said and when. Draft the reply in chat and let me approve or post it. Applies on any channel, not just GitHub (Slack, email, tickets).
