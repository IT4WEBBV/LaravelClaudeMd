...
4. **MANDATORY: Confirm the selection before continuing.** After the user picks agents, echo back the exact list you will dispatch to:
   ...
   > Dispatching to: **claude-opus**, **copilot**, **opencode**
...
Examples:
- `--tools claude-opus,copilot,opencode`
- `--group smart` (uses the configured group)
- `--group smart --tools opencode` (group plus explicit tools)
...
Example:
- `counselors loop --preset hotspots "critical request path" --tools claude-opus,copilot,opencode --duration 20m --json`
