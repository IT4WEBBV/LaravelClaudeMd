...
7. **Resume.** Inputs from your brief, progress from Step 1. A dead orchestrator's runs show as orphaned: one batched question. Re-arm watches and `notify_when_idle`, tear down merged, dispatch. After a compaction your dispatch record (task id → issue) still names each run, and its completion notice still arrives.
...
- **Suspected stall** (no notice, no commit or PR change for 90 minutes): `TaskStop` its workflow, which kills the running step's command and starts no further step; `finish` it as a halt (commands §Finish); ask *resume* / *leave it out*. *Resume* is a new `launch` and workflow.
...
|---|---|
| A fresh run, or "stop it, resume in a new workflow", for a quiet run; a second workflow beside one | A live suite looks identical; two runs in one worktree revert each other. |
