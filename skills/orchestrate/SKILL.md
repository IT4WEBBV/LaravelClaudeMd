...
7. **Resume.** Inputs from your brief, progress from Step 1. A dead orchestrator's runs show as orphaned: one batched question. Re-arm watches and `notify_when_idle`, tear down merged, dispatch. After a compaction: `auto`, message your agent ids; `autoflow`, your dispatch record (workflow run id → issue) still names each run and its completion notice still arrives.
